...
- **Tối ưu hóa Kiểm thử**:
  ...
  - Chuẩn hóa factory `createMockAvatar` để đảm bảo tương thích với interface `AvatarData` mở rộng.
- **Chuẩn hóa Giao diện**:
  - Cập nhật `AutomationView.vue` với các metrics trực quan và nhật ký hoạt động chi tiết.
- **Làm giàu dữ liệu & Đa ngôn ngữ**:
  - Tinh chỉnh logic tính toán World Boss/Event Dungeon khớp với block-range thực tế.
  - Hoàn thiện `resolveNameFromCsv` hỗ trợ đa ngôn ngữ linh hoạt (locale-aware) và cơ chế fallback thông minh.
  - Áp dụng **Lazy Name Resolution** tại lớp UI (`infoAllAvatarAddress.vue`, `AvatarDetailView.vue`) để giải quyết triệt để vấn đề đơ lag khi đổi ngôn ngữ.
  - Bổ sung trường `index` và đồng bộ hóa metadata cho `Equipment`, `Costume`, `Rune` tương thích hoàn toàn với dự án mẫu.

## Các quyết định quan trọng

- **Lazy Data Enrichment**: Di chuyển việc dịch thuật tên vật phẩm từ store (đồng bộ hàng loạt) sang component (lazy loading) để tối ưu hiệu năng Main Thread.
