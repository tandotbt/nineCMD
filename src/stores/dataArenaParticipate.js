...
export const useDataArenaParticipateStore = defineStore('dataArenaParticipateStore', () => {
    ...
    const getSeason = (url) => {
        return useFetch(url, { refetch: true }, {
            ...
            onFetchError(ctx) {
                // ctx.data can be null when 5xx response
                if (ctx.data === null)
                    ctx.data = selectedPlanet.value === "odin" ? encodeURIComponent("Season 19") : encodeURIComponent("Season 1")
    ...
    const idPlanetNow = computed(() => CONFIG_URL_ALL_PLANET.find(item => item.name.toLowerCase() === selectedPlanet.value).id)
    const urlGetSeason = computed(() => API_URL_PROXY + encodeURIComponent(`${API_NINE_CHRONICLES}/arena/season?planetId=${idPlanetNow.value}`))
    const { data: dataGetSeason } = getSeason(urlGetSeason)
    const urlArenaParticipate = computed(() => {
        if (!isUseAvatartLogin.value) {
            return `${urlRest9cscan.value}${path9cscanRest.value}${address.value}`
        } else if (isUseAvatartLogin.value) {
            return API_URL_PROXY + encodeURIComponent(`${API_NINE_CHRONICLES}/arena?planetId=${idPlanetNow.value}&season=${dataGetSeason.value}&userSet=1`)
        } else return null
    })
