...
async function scan() {
  ...
  const { GQL_QUERIES, CHARACTER_CODE_GETS, API_URLS } = await jiti.import(
    ...
    for (const key in obj) {
      ...
      if (typeof value === 'string') {
        ...
        if (isQuery || isMutation || isFragment) {
          ...
          if (
            trimmedValue.toLowerCase().includes('actionpoint') ||
  ...
  console.log(
    `Found ${registry.graphql.headless.length} Headless queries and ${registry.graphql.mimir.length} Mimir queries.`,
  )
}
