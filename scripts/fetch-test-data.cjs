...
async function main() {
  ...
  try {
    ...
    if (response.data) {
      fs.writeFileSync(path.join(FIXTURES_DIR, 'planets.json'), JSON.stringify(response.data, null, 2));
  ...
  for (const config of TEST_CONFIGS) {
    ...
    const fullDetailQuery = CHARACTER_QUERIES.GET_AVATAR_DETAIL_FULL(config.avatarAddress, config.agentAddress);
    const fullDetailData = await fetchGraphql(config.headlessUrl, fullDetailQuery);
    if (fullDetailData) {
      fs.writeFileSync(path.join(planetDir, 'headless.json'), JSON.stringify(pruneData(fullDetailData), null, 2));
      console.log('✓ Saved headless.json');
    }

    // Helper to find query in registry
    const findQuery = (name) => {
      return registry.graphql.headless.find(e => e.name === name) || 
    ...
    if (blocksQueryLatest) {
      ...
      if (data) {
        fs.writeFileSync(path.join(planetDir, `BLOCKS_GET_LATEST_MIMIR.json`), JSON.stringify(pruneData(data), null, 2));
    ...
    try {
      const url = get9cmdApiUrl(registry.rest.api9cmd.baseUrl, config.avatarAddress, config.planet, registry.rest.api9cmd.params);
      const response = await axios.get(url, { timeout: 10000 });
      if (response.data && response.data.data) {
        fs.writeFileSync(path.join(planetDir, '9cmd.json'), JSON.stringify(pruneData(response.data.data), null, 2));
    ...
    try {
      ...
      const response = await axios.get(url, { timeout: 10000 });
      if (response.data) {
        fs.writeFileSync(path.join(planetDir, 'season.json'), JSON.stringify(pruneData(response.data), null, 2));
